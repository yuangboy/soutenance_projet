        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            ...
            <div class="card-img-overlay">
              <h5 class="card-title">Liste Praticiens </h5>
              <p class="card-text"></p>
              <table class="table">
                        <thead>
                                <div class="alert alert-primary alert-dismissible fade show" role="alert">
                                    <i class="bi bi-star me-1"></i>
                                         <a href="/create" class = "btn btn-primary">Ajouter un praticien</a>
                        ...
                        <tbody>
                                @foreach ($praticiens as $praticiens)
                                            <tr>
                                                <td>{{$praticiens->id}}</td>
                                                <td>{{$praticiens->user_id}}</td>
                                                <td>{{$praticiens->service_id}}</td>
                                                <td>
                                                    <img src="{{ asset($praticiens->image) }}" alt="" width="100">
                                                </td>
                                                <td>{{$praticiens->prenom}}</td>
                                                <td>{{$praticiens->dateNaiss}}</td>
                                                <td>{{$praticiens->genre}}</td>
                                                <td>{{$praticiens->tel}}</td>
                                                <td>{{$praticiens->profession}}</td>
                                                <td>{{$praticiens->adresse}}</td>
                                                <td>{{$praticiens->mattricule}}</td>
                                                <td>{{$praticiens->specialite}}</td>
                                                <td>
                                                    <a href="{{ route('praticiens.edit', $praticiens->id) }}" class="btn btn-sm btn-warning">Modifier</a>
                                                    <td>
                                                    <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete({{ $praticiens->id }})">Supprimer</button>
                                                    <form action="{{ route('praticiens.destroy', $praticiens->id) }}" method="POST" style="display:none;" id="delete-form-{{ $praticiens->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                    </td>
          ...
          <script>
            function confirmDelete(praticienId) {
                if (confirm('Êtes-vous sûr de vouloir supprimer ce praticien ?')) {
                    document.getElementById('delete-form-' + praticienId).submit();
                }
            }
        </script>
